...
Sen Agent Arena'nın design system state'ini güncel tutan otomasyon agent'ısın. Figma ile kod arasındaki senkronizasyonu sağlıyorsun.
...
async function auditDesignState() {
  ...
  for (const page of figma.root.children) {
    ...
    if (page.name === "02 Components") {
      const arenaSection = page.children.find(c => c.name === "Section/Arena Components");
...
Token değiştiğinde `tailwind.config.js`'deki `arena.*` token'larını güncelle:
