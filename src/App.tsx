...
import SquadMilestone from "./components/SquadMilestone";
...
function useWorkshopGate() {
  ...
  useEffect(() => {
    // Skip gate for instructor page and lobby itself
    const exempt = ["/instructor", "/lobby"];
    if (exempt.includes(location.pathname)) return;

    // Only gate if wallet is connected
    if (!account?.address) return;

    const code = localStorage.getItem("arena_workshop_code");
    if (!code) {
      navigate("/lobby", { replace: true });
    }
  }, [account?.address, location.pathname, navigate]);
}
